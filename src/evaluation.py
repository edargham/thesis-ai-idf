...
class IDFModelEvaluator:
    ...
    def compare_models(self):
        ...
        for model_name, model_data in self.model_data.items():
            ...
            for duration_min in historical_return_periods.keys():
                if duration_min in model_data.index:
                    ...
                    for return_period in [2, 5, 10, 25, 50, 100]:
                        if return_period in historical_return_periods[duration_min]:
                            historical_values.append(
                                historical_return_periods[duration_min][return_period]
                            )
                            ...
                            if f"{return_period}-year" in model_data.columns:
                                model_values.append(
                                    model_data.loc[
                                        duration_min, f"{return_period}-year"
                                    ]
                                )
                    ...
                    if len(historical_values) > 0 and len(model_values) > 0:
                        ...
                        # Calculate metrics
                        rmse = np.sqrt(
                            mean_squared_error(historical_values, model_values)
                        )
                        mae = mean_absolute_error(historical_values, model_values)
                        r2 = r2_score(historical_values, model_values)
                        nse = nash_sutcliffe_efficiency(historical_values, model_values)
                        ...
                        model_results["nse"][duration_min] = nse
            ...
            if n_comparisons > 0:
                ...
                avg_nse = total_nse / n_comparisons
                avg_bias = total_bias / n_comparisons

                # Overall score (lower is better for RMSE, MAE, bias; higher is better for NSE)
    ...
    def create_visualizations(
        self, comparison_results, historical_stats, historical_return_periods
    ):
        ...
        composite_scores = [
            ...
            for model in models
            if comparison_results[model]["overall_score"]
        ]

        bars = ax1.bar(
            models, composite_scores, color=["blue", "green", "red", "orange", "purple"]
        )
        ...
        bars = ax2.bar(
            models, nse_scores, color=["blue", "green", "red", "orange", "purple"]
        )
        ax2.set_title(
            "Nash-Sutcliffe Efficiency\n(Higher is Better)", fontweight="bold"
        )
        ax2.set_ylabel("NSE Score")
        ...
        bars = ax3.bar(
            models, rmse_scores, color=["blue", "green", "red", "orange", "purple"]
        )
        ...
        for duration in durations:
            if duration in self.historical_data.columns:
                ax4.hist(
                    self.historical_data[duration], alpha=0.6, label=duration, bins=15
                )
        ...
        # 5. Model predictions vs historical estimates for 30 minutes
        ax5 = axes[1, 1]
        duration_min = 30  # 30 minutes instead of 1 hour
        if duration_min in historical_return_periods:
            return_periods = [2, 5, 10, 25, 50, 100]
            historical_vals = [
                historical_return_periods[duration_min].get(rp, np.nan)
                ...
                if duration_min in model_data.index:
                    ...
                    for rp in return_periods:
                        if f"{rp}-year" in model_data.columns:
                            model_vals.append(
                                model_data.loc[duration_min, f"{rp}-year"]
                            )
            ...
            ax5.set_title("30-Minute Duration Comparison", fontweight="bold")
        ...
        ax6 = axes[1, 2]
        bias_scores = [
            comparison_results[model]["overall_score"]["bias"]
            for model in models
            if comparison_results[model]["overall_score"]
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            "figures/model_evaluation_comprehensive.png", dpi=300, bbox_inches="tight"
        )
        plt.show()
    ...
    def generate_detailed_report(self, comparison_results, historical_stats):
        ...
        for duration_min, data_stats in historical_stats.items():
            duration_label = duration_labels.get(
                duration_min, f"{duration_min} minutes"
            )
        ...
        for model, results in comparison_results.items():
            if results["overall_score"]:
                ...
                print(f"  NSE Score:   {results['overall_score']['nse']:.4f}")
                ...
                print(
                    f"  Composite:   {results['overall_score']['composite_score']:.4f}"
                )
        ...
        print("\n   Key Strengths:")
        print(f"   • NSE Score: {best_results['nse']:.4f} (Nash-Sutcliffe efficiency)")
        print(f"   • RMSE: {best_results['rmse']:.2f} mm/h")
        print(f"   • MAE: {best_results['mae']:.2f} mm/h")
        print(
            f"   • Bias: {best_results['bias']:.2f} mm/h ({'overestimates' if best_results['bias'] > 0 else 'underestimates'})"
        )

        print("\n   Reliability Assessment:")
        if best_results["nse"] > 0.75:
            print("   ✓ Excellent Nash-Sutcliffe efficiency")
        elif best_results["nse"] > 0.5:
            print("   ✓ Good Nash-Sutcliffe efficiency")
        elif best_results["nse"] > 0:
            print("   ⚠ Moderate Nash-Sutcliffe efficiency")
        else:
            print("   ⚠ Poor Nash-Sutcliffe efficiency")
        ...
        if "Gumbel" in comparison_results:
            gumbel_score = comparison_results["Gumbel"]["overall_score"][
                "composite_score"
            ]
    ...
    def run_evaluation(self):
        ...
        # Perform comparison
        comparison_results, historical_stats, historical_return_periods = (
            self.compare_models()
        )
        ...
        # Generate report
        best_model, best_results = self.generate_detailed_report(
            comparison_results, historical_stats
        )
        ...
        results_df = pd.DataFrame(
            {
                model: {
                    "R2": (
                        results["overall_score"]["r2"]
                        if results["overall_score"]
                        else np.nan
                    ),
                    "NSE": (
                        results["overall_score"]["nse"]
                        if results["overall_score"]
                        else np.nan
                    ),
                    "RMSE": (
                        results["overall_score"]["rmse"]
                        if results["overall_score"]
                        else np.nan
                    ),
                    "MAE": (
                        results["overall_score"]["mae"]
                        if results["overall_score"]
                        else np.nan
                    ),
                    "Bias": (
                        results["overall_score"]["bias"]
                        if results["overall_score"]
                        else np.nan
                    ),
                    "Composite_Score": (
                        results["overall_score"]["composite_score"]
                        if results["overall_score"]
                        else np.nan
                    ),
                }
