...
class IDFDataset(Dataset):
    ...
    def prepare_data(
        self, annual_max_data: pd.DataFrame, scalers: tuple = None, fit_scalers: bool = True
    ):
        """
...
def evaluate_model(
    ...
    for i, rp in enumerate(return_periods):
        target_intensities = idf_target_data[idf_target_data["Return Period (years)"] == rp][duration_cols].values.flatten()
...
def generate_smooth_idf_curves(
    ...
    for return_period in return_periods:
        factor = frequency_factors[return_period]
        smooth_idf_curves[return_period] = np.array([intensity * factor for intensity in base_intensities])
...
def create_smooth_individual_plots(results: dict, model: LightweightAttentionTCN, dataset: Dataset):
    """
...
def main():
    ...
    results = {
        "TCAN": (rmse, mae, r2, nse, predicted_curves)
    }
    ...
    print("=" * 80)
    print(f"{'Model':<25} {'Parameters':<12} {'R² Score':<12} {'RMSE':<12} {'MAE':<12}")
    print("-" * 80)
    print(f"{'TCAN':<25} {param_count:<12} {r2:<12.6f} {rmse:<12.4f} {mae:<12.4f}")
