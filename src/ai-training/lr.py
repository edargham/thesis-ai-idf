...
log_empirical_durations = np.log(empirical_durations).reshape(-1, 1)
log_empirical_durations_scaled = scaler_X.transform(log_empirical_durations)

# Base prediction
base_log_empirical_intensities_scaled = model.predict(log_empirical_durations_scaled)
base_log_empirical_intensities = scaler_y.inverse_transform(
    base_log_empirical_intensities_scaled.reshape(-1, 1)
).flatten()
base_empirical_intensities = np.exp(base_log_empirical_intensities)

# Create dictionary to store model predictions for each return period
lr_idf_curves = {}
for rp in return_periods:
    lr_idf_curves[rp] = base_empirical_intensities * frequency_factors[rp]

# Map durations to column names in the empirical data
duration_column_mapping = {
    5: "5 mins",
    10: "10 mins", 
    15: "15 mins",
    30: "30 mins",
...
for rp in return_periods:
    ...
    for i, duration in enumerate(empirical_durations):
        column_name = duration_column_mapping[duration]
        y_true.append(empirical_row[column_name])
        y_pred.append(lr_idf_curves[rp][i])
...
for i, rp in enumerate(return_periods):
    ...
    plt.plot(empirical_durations, lr_idf_curves[rp], '-', color=colors[i], 
             linewidth=2, label=f"Model T = {rp} years")
    
    # Empirical data (dashed line with markers)
    empirical_row = empirical_idf[empirical_idf["Return Period (years)"] == rp].iloc[0]
    empirical_values = [empirical_row[duration_column_mapping[d]] for d in empirical_durations]
...
plt.tight_layout()
plt.savefig(
    os.path.join(os.path.dirname(__file__), "..", "figures", "idf_comparison_lr.png"),
    dpi=300,
)

# Generate smooth curves for original plot
