...
def calculate_empirical_return_periods(annual_max_data):
    ...
    for duration in durations:
        ...
        sorted_data = annual_max_data.sort_values(by=duration, ascending=False).reset_index(drop=True)
        n = len(sorted_data)
...
duration_mapping = {'30mns': 0.5, '1h': 1.0, '3h': 3.0, '24h': 24.0}
...
class IDFDataset(Dataset):
    def __init__(self, dataframe, duration_col='duration_hours', rp_col='return_period', intensity_col='intensity'):
        ...
        self.scaler_rp = StandardScaler()
        self.scaler_intensity = StandardScaler()
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(features, dtype=torch.float32), torch.tensor(self.y[idx], dtype=torch.float32)
...
train_dataset, test_dataset = torch.utils.data.random_split(idf_dataset, [train_size, test_size])
...
durations_hours = [0.5, 1, 3, 24]  # 30min, 1h, 3h, 24h
...
display_df = pd.DataFrame({
    ...
    '30min': [idf_curves[rp][0] for rp in return_periods],
    '1h': [idf_curves[rp][1] for rp in return_periods],
    '3h': [idf_curves[rp][2] for rp in return_periods],
    '24h': [idf_curves[rp][3] for rp in return_periods]
})
...
plt.grid(True)
plt.tight_layout()
plt.savefig('neural_network_idf_curves.png', dpi=300, bbox_inches='tight')

# Optional: Compare with traditional statistical approaches
# For example, implementing Gumbel or GEV distribution fitting
