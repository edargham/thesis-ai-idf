...
def main():
  ...
  csv_path = "./data/beirut-era-temps.csv"
  ...
  daily_precip_max = df.groupby(df['date'].dt.floor("d"))["value"].max().reset_index()
  ...
  daily_precip_max.to_csv("./data/beirut-daily-temps-max.csv", index=False)
  ...
  print("Daily precipitation accumulation has been saved to 'beirut-daily-temps-max.csv'.")
